<template>
  <div class="flex flex-col gap-0.5">
    ...
    <!-- 5h窗口限制 (固定金额 or utilization%) -->
    <CapacityBadge v-if="showWindowCost" :color-class="windowCostClass" :tooltip="windowCostTooltip" :current="windowCostCurrent" :max="windowCostMax">
      <svg class="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
const isUtilizationMode = computed(() => {
  const limit = Number(extra.value.window_utilization_limit || 0)
  return limit > 0
})
...
const windowCostCurrent = computed(() => {
  if (isUtilizationMode.value) {
    return Math.round(currentUtilization.value * 100) + '%'
  }
  return '$' + formatCost(currentWindowCost.value)
})

const windowCostMax = computed(() => {
  if (isUtilizationMode.value) {
    return Math.round(utilizationLimit.value * 100) + '%'
  }
  return '$' + formatCost(props.account.window_cost_limit || 0)
})

const windowCostClass = computed(() => {
  if (!showWindowCost.value) return ''
  if (isUtilizationMode.value) {
    const current = currentUtilization.value
    const limit = utilizationLimit.value
    const reserve = utilizationReserve.value

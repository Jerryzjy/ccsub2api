<template>
  ...
  >
    ...
    >
      ...
      <!-- Claude Subscription Tier (Anthropic OAuth only) -->
      <div
...
function loadQuotaControlSettings(account: Account) {
  ...
  if (account.omit_billing_attribution === true) {
    omitBillingAttribution.value = true
  }
  ...
  try {
    ...
    if (props.account.platform === 'anthropic' && (props.account.type === 'oauth' || props.account.type === 'setup-token')) {
      ...
      if (omitBillingAttribution.value) {
        newExtra.omit_billing_attribution = true
      } else {
        delete newExtra.omit_billing_attribution
      }
    ...
    // Claude subscription tier (Anthropic OAuth/Setup Token)
    if (props.account.platform === 'anthropic' && (props.account.type === 'oauth' || props.account.type === 'setup-token')) {
      const currentExtra = (updatePayload.extra as Record<string, unknown>) || (props.account.extra as Record<string, unknown>) || {}
      const newExtra = { ...currentExtra }
      if (claudeTier.value) {
        newExtra.claude_tier = claudeTier.value
      } else {
        delete newExtra.claude_tier
      }
      updatePayload.extra = newExtra
    }

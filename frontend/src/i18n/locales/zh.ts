  ...
  admin: {
    ...
    accounts: {
      ...
      quotaControl: {
        ...
        tpmLimit: {
          label: 'TPM 限制',
          hint: '限制每分钟消耗的 token 数（输入+输出+缓存），保护上游账号',
          baseTpm: '基础 TPM',
          baseTpmPlaceholder: '200000',
          baseTpmHint: '每分钟最大 token 数，0 或留空表示不限制。对大上下文流量比 RPM 更贴近上游真实限额',
          stickyBuffer: '粘性缓冲区',
          stickyBufferPlaceholder: '默认: base TPM 的 20%',
          stickyBufferHint: '超过 base TPM 后，粘性会话额外允许的 token 数。为空则使用默认值（base TPM 的 20%，最小为 1）',
        },
        outboundHeaders: {
          label: '出站 Header 覆盖',
          hint: '在转发到上游前，逐账号覆盖或删除请求头',
          warning: '⚠️ 此项会修改发往上游的请求头，可能破坏客户端指纹伪装、增加封号风险。authorization 与 x-api-key 受保护、无法覆盖。请仅在明确知道后果时使用。',
          overridesLabel: '覆盖 / 新增 Header',
          removesLabel: '删除 Header',
          namePlaceholder: 'Header 名（如 X-Pool-Key）',
      ...
      oauth: {
        ...
        authMethod: '授权方式',
        manualAuth: '手动授权',
        credentialsImport: '导入 credentials.json',
        credentialsImportDesc: '导入已生成的 Claude OAuth credentials.json，并直接创建 OAuth 账号。此功能不会把 Cookie 或 sessionKey 转换为 OAuth。',
        credentialsFile: 'credentials.json 文件',
        credentialsFileHint: '请选择包含 claudeAiOauth.accessToken 和 refreshToken 的 JSON 文件',
        importingCredentials: '导入中...',
        importCredentialsAndCreate: '导入并创建 OAuth 账号',
        credentialsInvalid: 'credentials.json 无效或缺少 OAuth 凭据',
        cookieAutoAuth: 'sessionKey 自动授权',
        cookieAutoAuthDesc: '使用 claude.ai sessionKey 获取当前选择的 OAuth 或 Setup Token 权限；仅在 Anthropic 接受该会话时成功。',
        ...
        startAutoAuth: '使用 sessionKey 授权',
        authorizing: '授权中...',

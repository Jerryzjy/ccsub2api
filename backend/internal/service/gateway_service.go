...
func (s *GatewayService) GenerateSessionHash(parsed *ParsedRequest) string {
	...
	// 3. 最后 fallback: 使用 session上下文 + system + 第一条 user 消息
	// 只用第一条 user 消息而非所有消息，保证同一会话多轮对话的 hash 稳定，
	// 从而维持 sticky session 绑定，提升 prompt cache 命中率。
	...
	appendFirstUserMessageFromRaw(&combined, parsed.MessagesRaw())
...
// appendFirstUserMessageFromRaw 只提取第一条 user 消息的文本，用于生成稳定的 session hash。
// 同一会话中第一条 user 消息通常不变，确保多轮对话的 hash 一致。
func appendFirstUserMessageFromRaw(builder *strings.Builder, raw []byte) {
	if builder == nil || len(raw) == 0 {
		return
	}
	messages := parseRawJSONView(raw)
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	if shouldMimicClaudeCode {
		...
		// 第三方客户端通常不发 cache_control，无条件注入 message cache breakpoints
		// 以确保多轮对话的缓存命中率。
...
func (s *GatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool) (*http.Request, []byte, error) {
	...
	// 移除不支持 assistant prefill 的模型的空 assistant 消息
	// 防止 "This model does not support assistant message prefill" 400 错误
	if stripped, err := stripAssistantPrefillIfUnsupported(body, modelID); err == nil {
		body = stripped
	}
...
func applyClaudeCodeMimicHeaders(req *http.Request, isStream bool) {
	...
	// 归一化 accept-encoding，避免不同客户端产生行为不一致的指纹
	setHeaderRaw(req.Header, "accept-encoding", "gzip, deflate")
}
